...
class PDFLabelProcessor:
    def __init__(self, threshold=200, padding_percent=2, dpi=300, aspect_lock=True, border_padding_percent=10):
        self.threshold = threshold
        self.padding_percent = padding_percent
        self.dpi = dpi
        self.aspect_lock = aspect_lock
        self.border_padding_percent = border_padding_percent
    ...
    def find_label_border(self, img):
        ...
        for c in contours:
            ...
            edge_score = (left.mean() + right.mean() + top.mean() + bottom.mean()) / 255.0
            if edge_score < 0.05:
                continue
            if area > best_area:
                best_area = area
    ...
    def _process_core(self, img):
        ...
        if border_rect is not None:
            ...
            pad_x = max(1, int(round(w * self.border_padding_percent / 100.0)))
            pad_y = max(1, int(round(h * self.border_padding_percent / 100.0)))
            x = max(0, x - pad_x)
            y = max(0, y - pad_y)
            w = min(img.shape[1] - x, w + 2 * pad_x)
            h = min(img.shape[0] - y, h + 2 * pad_y)
            used_border = True
            label_prefix = f"Detected (border+{self.border_padding_percent}%)"
